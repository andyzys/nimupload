...
(function (root, factory) {
  ...
  Uploader.prototype.checkPending = function() {
    let checked = false
    $.each(this._.options.fileList, function(i, v) {
    ...
    if(!context) {
      $.ajax({
        ...
        headers: {
          ...
          'Nonce'   : self._.options.Nonce, //随机数（随机数，最大长度128个字符）
          'CurTime' : self._.options.CurTime, //当前UTC时间戳，从1970年1月1日0点0分0秒开始到现在的秒数
  ...
  Uploader.prototype.onUploaded = function(curFile) {
    ...
    $.ajax({
      ...
      headers    : {
        ...
        'Nonce': options.Nonce, //随机数（随机数，最大长度128个字符）
  ...
  Uploader.prototype.upload = function(fileIdx) {
    let self = this
    let fileList = this._.options.fileList
  ...
  Uploader.prototype.initEvent = function() {
    ...
    $('#' + this._.options.fileUploadId).on('click', function() {
