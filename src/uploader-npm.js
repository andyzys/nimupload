...
let defaultOpts = {
  ...
  onProgress: function(curFile) {
    console.log('info', '传输状态：'+curFile.status + '；进度：'+curFile.progress)
  },
  onFinished: function(curFile) {
    log('info', 'File: ' + curFile.fileName + ' is uploaded.')
  }
}

let Uploader = function(optsObj) {
  ...
  let options = {
    ...
    CheckSum     : optsObj['CheckSum'],
    CurTime      : optsObj['CurTime'],
    Nonce        : optsObj['Nonce'],
    /**
     * 上传输入框元素ID
     * @type {[String]}
  ...
  // deep clone object
  for (let key in defaultOpts) this._.options[key] = defaultOpts[key]
...
Uploader.prototype.addFileToList = function(fileInput, cb) {
  ...
  this._.options.fileList.push(fileObj)
...
Uploader.prototype.getInitInfo = function(file, cb) {
  let self = this
  let context = localStorage.getItem(file.fileKey + '_context')
  if(!context) {
    request
      .post('http://vcloud.163.com/app/vod/upload/init')
      //headers参数为API token校验返回的结果，全部均为必须
      .set('AppKey', self._.options.AppKey)//开发者平台分配的appkey
      .set('Nonce', self._.options.Nonce)//随机数（随机数，最大长度128个字符）
      .set('CurTime', self._.options.CurTime)////当前UTC时间戳，从1970年1月1日0点0分0秒开始到现在的秒数
      .set('CheckSum', self._.options.CheckSum)//服务器认证需要,SHA1(AppSecret+Nonce+CurTime),16进制字符小写
      .send({
...
Uploader.prototype.getDNS = function(param, cb) {
  ...
  if (options.dnsList) { //已缓存则直接取缓存数据
    ...
    request
      .get(options.urlDns)
      .query({ bucketname: param.bucketName })
      .query({ version: '1.0' })
...
Uploader.prototype.getOffset = function(param, cb) {
  let context = localStorage.getItem(param.fileKey + '_context')
  let options = this._.options
  if (!context) return cb(0)
  request
    .get(param.serveIp + '/' + param.bucketName + '/' + param.objectName + '?uploadContext')
    .query({ context: context })
    .query({ version: '1.0' })
    .set('x-nos-token', param.nosToken)
    .end(function(err, res){
      if(err || !res.ok) {
        options.onError(err)
      } else {
...
Uploader.prototype.removeFile = function(file) {
  ...
  fileList.map((fileObj, index) => {
    if (fileObj.fileKey === file.fileKey) {
      if (fileObj.xhr) {
        fileObj.xhr.upload.onprogress = ()=>{}
        fileObj.xhr.onreadystatechange = ()=>{}
        fileObj.xhr.abort()
        fileObj.xhr = null
      }
...
Uploader.prototype.onUploaded = function(curFile) {
   ...
   * */
  request
    .post('http://vcloud.163.com/app/vod/video/query')
    .set('AppKey', options.AppKey)
    .set('Nonce', options.Nonce)
    .set('CurTime', options.CurTime)
    .set('CheckSum', options.CheckSum)
    .send({
...
Uploader.prototype.upload = function(fileIdx) {
  ...
  if(fileIdx < fileList.length) {
    ...
    // todo
    this.getInitInfo(fileList[fileIdx], function(data) {
      let curFile = fileList[fileIdx]
      curFile.objectName = data.objectName
      curFile.bucketName = data.bucketName

      self.getDNS(data, function(dnsList) {
        let curFile = fileList[fileIdx]
        self.getOffset({
          serveIp   : dnsList[0],
...
Uploader.prototype.initEvent = function() {
  ...
  document.getElementById(this._.options.fileInputId).addEventListener('change', function(e) {
    let chooseFileExt = ''
    if(e.target.files) {
      if(!self.checkExistInFileList(e.target.files[0])) {
        chooseFileExt = e.target.files[0].name.split('.').pop().toUpperCase()
        if (self._.options.fileExts.indexOf(chooseFileExt.toUpperCase()) < 0) {
          log('warn', '请选择视频或图片文件：不是有效的视频或图片格式！')
          return
        }
        self.addFileToList(e.target, function(fileInput, fileObj) {
  ...
  document.getElementById(this._.options.fileUploadId).addEventListener('click', function() {
...
/**
 * extend对象
 * @param  {Object} out extend后返回的对象
 * @return {Object} out    extend后返回的对象
 */
function extend(out) {
  out = out || {}
